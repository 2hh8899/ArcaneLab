ig.module("impact.feature.base.event-steps.arcane-lab-event-commands1").requires("impact.feature.base.event-steps").defines(function() {
	...
	ig.EVENT_STEP.AL_OPEN_TELEPORT_MENU = ig.EventStepBase.extend({
		hub: null,
		_wm: new ig.Config({
			attributes: {}
		}),
		init: function() {},
		start: function() {
			sc.menu.setDirectMode(true, sc.MENU_SUBMENU.AL_TELEPORTER);
			sc.model.enterMenu(true);
			sc.model.prevSubState = sc.GAME_MODEL_SUBSTATE.RUNNING
		}
	});
	ig.EVENT_STEP.AL_OPEN_CUSTOM_SKILL_MENU = ig.EventStepBase.extend({
		hub: null,
		_wm: new ig.Config({
			attributes: {}
		}),
		init: function() {},
		start: function() {
			sc.menu.setDirectMode(true, sc.MENU_SUBMENU.AL_CUSTOM_SKILL);
			sc.model.enterMenu(true);
			sc.model.prevSubState = sc.GAME_MODEL_SUBSTATE.RUNNING
		}
	});
